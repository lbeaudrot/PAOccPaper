...
```{r}
simulatetrend <- function(points = 30,days = 30, psi = 0.5, p = 0.2, phi = 0.2, gamma = 0, years = 5, nsim = 5) {
         #store results
...
```{r, cache = TRUE}
test <- simulatetrend(points = 100, psi = 0.9, gamma = 0, phi = 0.95, nsim = 100)
```
...
```{r}
sl <- data.frame(test$linear)
ggplot(sl,aes(x=X2)) + geom_histogram()
```